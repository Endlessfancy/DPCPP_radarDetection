...
void Find2dAbsMax(MKL_Complex16 **ptr, int rowsize, int colsize, int *maxidx){
    maxidx[0] = 0;
    maxidx[1] = 0;
    double maxval = 0;
    double absval;
...
void printComplex(MKL_Complex16 *Data, int size, char string[]){
    printf("Print %s!\n", string);
    for (int i = 0; i < size; i++){
...
int main() {
  try {
    ...
    double *Dis = malloc_shared<double>(91, q);
    double *Spd = malloc_shared<double>(91, q);
    double *Agl = malloc_shared<double>(91, q);
    double *SDis = malloc_shared<double>(91, q);
    int *maxDisIdx = malloc_shared<int>(91, q);
    int *maxSpdIdx = malloc_shared<int>(91, q);
    int *maxSDisIdx = malloc_shared<int>(91, q);
    int *maxAglIdx = malloc_shared<int>(91, q);
    ...
    while(1){
        ...
        // printComplex(DataShared_2dFFT[0], 10, "in");
        for(int chp = 0; chp < ChirpSize_extend; chp++){
            oneAPI_FFT_inplace(DataShared_2dFFT[chp],  SampleSize_extend);
        }
        // printComplex(DataShared_2dFFT[0], 128, "fft row");
        ...
        Matrix_Transpose(DataShared_2dFFT, DataShared_2dFFT_tp, ChirpSize_extend, SampleSize);
        ...
        double maxval = 0;
        double absval= 0;
        for (int i = 0; i < 0.4* SampleSize_extend; i++){
          for (int j = 0; j <  ChirpSize_extend ; j++){
            absval = Complex_mol((&DataShared_2dFFT_tp[i][j]));
            if(absval > maxval){
              maxval = absval;
              maxSDisidx = i;
              maxSpdidx = j;
        ...
        // int maxSDisidx = max2didx[0];
        // int maxSpdidx = max2didx[1];
